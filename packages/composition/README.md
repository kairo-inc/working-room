...
A single `src/index.ts` builds the container: data sources (`*Source`), the local `FileAccessService`/`BlobStore` pair, the `ChatEngine` and `EventBus`, the default Agents (`agentCoordinator`, `agentHeavy`), and the core Tools (e.g. `ToolReadTextFile`, `ToolWriteReplace`, `ToolListDir`).
