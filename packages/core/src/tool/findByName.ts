...
const inputSchema = z.object({
  searchName: z.string().describe("The file/folder name to search for."),
})

@injectable()
export class ToolFindFileByName extends Tool {
  name = "ToolFindFileByName"
  description = `Find files which name contains the specified text.
  ...
  async run({ toolCall, ctx: { depth } }: ToolRunArgs): Promise<ToolRunResult> {
    ...
    try {
      ...
      if (e instanceof NotFoundError) {
        return { message: this.buildError(toolCall, `No files found with the name: ${input.data.searchName}`) }
      }
      return { message: this.buildError(toolCall, `Failed to find files by name: ${(e as Error).message}`) }
    }
  }
}
