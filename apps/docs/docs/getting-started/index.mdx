...
- For all setups: at least one AI provider API key
  - `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, and/or `GOOGLE_GENERATIVE_AI_API_KEY`
  - At least one of these keys is required to run the application.
  - Alternatively, a self-hosted LLM can be used via an OpenAI-compatible API by setting `SELF_HOSTED_BASE_URL` and `SELF_HOSTED_API_KEY`.
