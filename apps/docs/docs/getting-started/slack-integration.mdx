...
WorkingRoom can connect a User's account to Slack via OAuth2, letting Agent Tools list Slack channels and send messages on that User's behalf. This integration is optional: if it is not configured, Users simply cannot connect Slack, and Slack-backed Tools have nothing to act on.
