...
const variants = cva(
  "rounded-md border border-border bg-input-background h-10 leading-5 px-4 text-base focus:border-transparent focus:ring-2",
  {
    variants: {
      variant: {
...
export interface SelectFormProps extends ComponentPropsWithoutRef<"select">, Variants {
...
export const SelectForm = forwardRef<HTMLSelectElement, SelectFormProps>(
  ({ className, variant = "default", formName, label, options, disabled, ...props }, ref) => {
    ...
    return (
      <div className="inline-flex w-full flex-col gap-1">
        ...
        <select id={formName} className={variants({ className, variant })} {...input} {...props} disabled={isDisabled} ref={ref}>
          {options.map((option) => (
