...
const Heading1 = ({ children, className }: React.ComponentProps<"h1">) => {
  return <h1 className={clsx("pt-4 pb-4 text-xl font-bold", className)}>{children}</h1>
}
const Heading2 = ({ children, className }: React.ComponentProps<"h2">) => {
  return <h2 className={clsx("pt-4 pb-4 text-base font-bold", className)}>{children}</h2>
}
const Heading3 = ({ children, className }: React.ComponentProps<"h3">) => {
  return <h3 className={clsx("pt-4 pb-4 text-base font-bold", className)}>{children}</h3>
}
const P = ({ children, className }: React.ComponentProps<"p">) => {
  return <p className={clsx("mb-4 text-sm", className)}>{children}</p>
}
const OrderedList = ({ children, className }: React.ComponentProps<"ol">) => {
  return <ol className={clsx("mb-4 list-decimal pl-6 text-sm", className)}>{children}</ol>
}
const UnorderedList = ({ children, className }: React.ComponentProps<"ul">) => {
  return <ul className={clsx("mb-4 list-disc pl-6 text-sm", className)}>{children}</ul>
}

const ListItem = ({ children, className }: React.ComponentProps<"li">) => {
  return <li className={clsx("text-sm", className)}>{children}</li>
}

const CodeBlock = ({ children, className }: React.ComponentProps<"code">) => {
  const isInline = !className?.includes("language-")
  if (isInline) {
    return <code className={clsx("bg-muted rounded-sm px-1 py-0.5 text-sm font-normal", className)}>{children}</code>
  }
  return (
    <pre className={clsx("bg-muted mb-4 overflow-x-auto rounded-sm p-4 text-sm", className)}>
...
const Hr = ({ className }: React.ComponentProps<"hr">) => {
  return <hr className={clsx("my-4 border-t", className)} />
}

const BlockQuote = ({ children, className }: React.ComponentProps<"blockquote">) => {
  return <blockquote className={clsx("border-l-2 py-2 pl-4 text-sm italic [&>p]:last:mb-0", className)}>{children}</blockquote>
}

const Anchor = ({ children, className, href }: React.ComponentProps<"a">) => {
  return (
    <a href={href} className={clsx("text-link hover:text-link-hover underline", className)} target="_blank" rel="noopener noreferrer">
      {children}
    </a>
  )
}
...
const Td = ({ children, className }: React.ComponentProps<"td">) => {
  return <td className={clsx("border px-4 py-2 text-sm whitespace-normal", className)}>{children}</td>
}
